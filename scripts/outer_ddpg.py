...
def train(sess, env, actor, critic):
    ...
    replay_memory = ActionReplayMemory(BUFFER_SIZE,RANDOM_SEED)
    ...
    for i in xrange(MAX_EPISODES):
        ...
        for j in xrange(MAX_EP_STEPS):
            ...
            replay_memory.insert(np.reshape(s, (actor.s_dim,)), np.reshape(s2, (actor.s_dim,)), np.reshape(a, (actor.a_dim,)), r,terminal)
            ...
            if replay_memory.size() > MINIBATCH_SIZE:
                ...
                s_batch, s2_batch, a_batch, r_batch, t_batch  = \
                    replay_memory.sample(MINIBATCH_SIZE)
                ...
                a_batch = np.reshape(np.array(a_batch),(MINIBATCH_SIZE,1))
                ...
                # print(s_batch.shape)
                # print(ts_batch.shape)
                # print(a_batch.shape)
                # print(ta_batch.shape)
                # print(len(a_batch))
                # print(len(y_i))
